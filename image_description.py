...
# !gdown --id 1IdaBtMSvtyzF0ByVaBHtvM0JYSXRExRX -O /pretrained_models/model_wieghts.pt
...
while(1):
    imageIdxInput = input("insert image index: ").strip()
    if((not imageIdxInput.isdigit()) or (int(imageIdxInput)>4)):
        raise ValueError("only 1~4 integer is available")
    imageIdx = int(imageIdxInput)

    if(not imageIdx):
        pil_image = capture_image()
    else:
        pil_image = PIL.Image.open(f'image{imageIdx}.jpg')
    # print(type(image))
    ...
    # # Optional: Play the file (only works if you have an audio player installed)
    os.system("start output.mp3")  # For Windows, use "open" for Mac, "xdg-open" for Linux
